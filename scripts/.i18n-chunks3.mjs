...
const OUT = join(ROOT, 'scripts', '.i18n-chunks3')
const IN_FLIGHT = ['chunk-01.txt', 'chunk-04.txt']
...
for (const f of IN_FLIGHT) {
  const text = readFileSync(join(ROOT, 'scripts', '.i18n-chunks2', f), 'utf8')
...
console.log(`${todo.length} messages (excluding ${claimed.size} in flight) -> ${n} chunks in scripts/.i18n-chunks3/`)
