...
function build$IMPLEMENTED(): ImplementedItem[] {
  ...
  { category: 'data-integration', title: t('File-backed persistence for alerts, incidents, decisions and the audit trail'), detail: t('A dev-server-only store (`scripts/state-persistence-plugin.ts`) that survives a reload - closing the most visible gap in a demonstration without claiming to be the production database below. Falls back cleanly to the in-session store when unavailable, including in any production build.') },
  ...
  { category: 'multi-tenant', title: t('A second sourced corporation record'), detail: t('Pune Municipal Corporation - area, population, budget, water supply and more, each field cited to a real published source in `corporations.ts`, following the same discipline Brihanmumbai\'s record uses. Fields with no citable source were left null rather than estimated. The roster generalising is now demonstrated, not merely asserted.') },
  { category: 'multi-tenant', title: t('Geography generator validated against a second city form'), detail: t('Pune is landlocked and riverine - the opposite city form to Brihanmumbai\'s coastline - and the schematic ward-tessellation generator has now actually produced a distinct, non-degenerate geography for it, checked by `scripts/preview-maps.mjs` and an automated test.') },
]
}
...
function build$PRODUCTION_REQUIREMENTS(): RequiredItem[] {
  ...
  { category: 'infrastructure', heldBy: 'platform-engineering', scale: 'months', title: t('Production database'), detail: t('A durable, backed-up, access-controlled data store - a real RDBMS on managed infrastructure, replacing the dev-only, single-machine, file-backed store this build now uses to survive a reload.') },
  { category: 'security', heldBy: 'platform-engineering', scale: 'months', title: t('Real append-only audit store'), detail: t('A tamper-evident, independently retained audit log on managed infrastructure. The current trail now survives a reload in development (see Implemented), but a git-ignored JSON file on one machine is not tamper-evident, backed up, or independently retained - it cannot alone satisfy an institutional assurance requirement.') },
  ...
  { category: 'multi-tenant', heldBy: 'corporation', scale: 'quarters', title: t('Per-corporation onboarding agreement'), detail: t('Each additional corporation the engine serves needs its own data-sharing, hosting and access agreement - the reusable engine lowers the engineering cost of a second deployment, not the institutional one. Pune, like Brihanmumbai, has none: this record demonstrates the engine, not a live PMC deployment.') },
]
}
